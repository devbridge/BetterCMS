        ...
        public void GetBlogPosts()
        {
            RunActionInTransaction(session =>
            {
                using (var api = CreateApi(session))
                {
                    ...
                    var request = new GetBlogPostsRequest(b => b.Category.Id == category.Id, includeUnpublished: true);
                ...
                session =>
                    {
                        using (var api = CreateApi(session))
                        {
                            ...
                            var request = new GetBlogPostsRequest(b => b.Category.Name == category.Name, includeUnpublished: true);
                            var response = api.GetBlogPosts(request);
            ...
            RunActionInTransaction(
               session =>
               {
                   using (var api = CreateApi(session))
                   {
                       ...
                       var request = new GetBlogPostsRequest(b => b.Category.Id == category.Id && b.Title.StartsWith("L"), includeUnpublished: true);
                       var response = api.GetBlogPosts(request);
            ...
            RunActionInTransaction(
               session =>
               {
                   using (var api = CreateApi(session))
                   {
                       ...
                       var request = new GetBlogPostsRequest(b => b.Category.Id == category.Id, b => b.Title, true, includeUnpublished: true);
                       var response = api.GetBlogPosts(request);
            ...
            RunActionInTransaction(
               session =>
               {
                   using (var api = CreateApi(session))
                   {
                       ...
                       var request = new GetBlogPostsRequest(b => b.Category.Id == category.Id, b => b.Title, startItemNumber: 3, itemsCount: 5, includeUnpublished: true);
                       var response = api.GetBlogPosts(request);
