...
namespace BetterCms.Module.MediaManager.Command.Audios
{
    public class GetAudiosCommand : CommandBase, ICommand<MediaManagerViewModel, MediaManagerItemsViewModel>
    {
        /// <summary>
        /// Executes this command.
        /// </summary>
        /// <param name="request">A filter to search for specific audios/folders.</param>
        ...
        public MediaManagerItemsViewModel Execute(MediaManagerViewModel request)
        {
            ...
            var audios = GetAudios(request);
            if (request.Direction == SortDirection.Descending)
            {
                results = audios.Cast<MediaViewModel>().Concat(folders);
            }
            else
            {
                results = folders.Cast<MediaViewModel>().Concat(audios);
            }
        ...
        private IList<MediaFolderViewModel> GetFolders(MediaManagerViewModel request)
        {
            ...
            var query = UnitOfWork.Session
                .QueryOver(() => alias)
                .Where(() => !alias.IsDeleted && alias.Type == MediaType.Audio);
        ...
        /// <returns>
        /// The list with audio view models
        /// </returns>
        private IList<MediaAudioViewModel> GetAudios(MediaManagerViewModel model)
        {
            MediaFile alias = null;
            MediaAudioViewModel modelAlias = null;
            ...
            query = query.SelectList(select => select
                    ...
                    .Select(() => alias.Title).WithAlias(() => modelAlias.Name)
                    .Select(() => alias.Version).WithAlias(() => modelAlias.Version))
                .TransformUsing(Transformers.AliasToBean<MediaAudioViewModel>());

            query.AddSortingAndPaging(model);

            var audios = query.List<MediaAudioViewModel>();
            return audios;
        }
        ...
        {
            var emptyFolderViewModel = new MediaFolderViewModel { Id = Guid.Empty, Type = MediaType.Audio };
