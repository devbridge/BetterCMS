...
namespace BetterCms.Module.MediaManager.Command.Videos
{
    public class GetVideosCommand : CommandBase, ICommand<MediaManagerViewModel, MediaManagerItemsViewModel>
    {
        ...
        /// </summary>
        /// <param name="request">A filter to search for specific videos/folders.</param>
        ...
        public MediaManagerItemsViewModel Execute(MediaManagerViewModel request)
        {
            ...
            var videos = GetVideos(request);
            if (request.Direction == SortDirection.Descending)
            {
                results = videos.Cast<MediaViewModel>().Concat(folders);
            }
            else
            {
                results = folders.Cast<MediaViewModel>().Concat(videos);
            }
        ...
        private IList<MediaFolderViewModel> GetFolders(MediaManagerViewModel request)
        {
            ...
            var query = UnitOfWork.Session
                .QueryOver(() => alias)
                .Where(() => !alias.IsDeleted && alias.Type == MediaType.Video);
        ...
        /// <returns>
        /// The list with video view models
        /// </returns>
        private IList<MediaVideoViewModel> GetVideos(MediaManagerViewModel model)
        {
            MediaFile alias = null;
            MediaVideoViewModel modelAlias = null;
            ...
            query = query.SelectList(select => select
                    ...
                    .Select(() => alias.Title).WithAlias(() => modelAlias.Name)
                    .Select(() => alias.Version).WithAlias(() => modelAlias.Version))
                .TransformUsing(Transformers.AliasToBean<MediaVideoViewModel>());

            query.AddSortingAndPaging(model);

            var videos = query.List<MediaVideoViewModel>();
            return videos;
        }
        ...
        {
            var emptyFolderViewModel = new MediaFolderViewModel { Id = Guid.Empty, Type = MediaType.Video };
